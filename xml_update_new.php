...
while ($arUser = $rsUsers->GetNext()) {
    ...
    $xml .= '<Партнер GUID_Партнера="'.$arUser["UF_GUID"].'" Наименование="' . str_replace(array("'", '"', '/'), '', $contact_fio) . '" Конкурент="Ложь">';
    ...
    */
    ...
    $xml .= '<КонтактныеЛицаПартнера>
                    <КонтактноеЛицо>
                        <GUID_Контакт>' . $arUser["UF_GUID"] . '</GUID_Контакт>
                        <ID>' . $contact_id . '</ID>
    ...
    while ($ar_sales = $db_sales->Fetch()) {
        ...
        while ($arPropVals = $db_propVals->Fetch()) {
            ...
            if ($ar_sales["PERSON_TYPE_ID"] == 1 or $ar_sales["PERSON_TYPE_ID"] == 6) {
                ...
                if ($arPropVals["PROP_CODE"] == "1C_GUID") $guid_1c = $arPropVals["VALUE"];
        ...
        if ($ar_sales["PERSON_TYPE_ID"] == 1 or $ar_sales["PERSON_TYPE_ID"] == 6) {
            $xml .= '<Контрагент>
                    <GUID_Контрагент>'.$guid_1c.'</GUID_Контрагент>
                    <ID>' . $ar_sales["ID"] . '</ID>
