...
if (!function_exists('getRangePercent')) {
    function getRangePercent($propValue)
    {
        $percent = 0;
        if (strstr($propValue, '/')) {
...
foreach ($ranges as $rangeCode) {
    if ($arResult['ITEM']['PROPERTIES'][$rangeCode]['VALUE']) {
        $percent = getRangePercent($arResult['ITEM']['PROPERTIES'][$rangeCode]['VALUE']);
        if ($percent > 0)
            $arResult['ITEM']['RANGES'][$rangeCode] = ['NAME' => $arResult['ITEM']['PROPERTIES'][$rangeCode]['NAME'], 'VALUE' => getRangePercent($arResult['ITEM']['PROPERTIES'][$rangeCode]['VALUE'])];
    }
}
