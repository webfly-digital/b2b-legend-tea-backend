(function () {
    ...
    window.JSWebflySaleProfile = function () {
        ...
        this.ajaxUrl = '/bitrix/templates/b2b/components/bitrix/sale.personal.profile.detail/personal/ajax.php';
        ...
        this.COMPANY_ADR_Field = BX("COMPANY_TITLE");
        this.COMPANY_TITLE_Field = BX("COMPANY_ADR");
    ...
    window.JSWebflySaleProfile.prototype = {
        bindEvents: function () {
            //  BX.bind(this.userIsContact, 'change', BX.proxy(this.userIsContactHandler, this));
        ...
        userIsContactHandler: function () {
            if (this.userIsContact.checked) {
                this.switchFields.forEach(field => {
                    field.setAttribute('readonly', true);
                });
                this.runComponentAction('setUserAsContact', {});
            } else {
                this.switchFields.forEach(field => {
        ...
        runComponentAction: function (action, data) {
            ...
            BX.ajax.runComponentAction('bitrix:sale.personal.profile.detail', action, {
                mode: 'class',
                data: data,
            }).then(function (response) {
                if (response.data.hasOwnProperty('ID')) {
                    _this.switchFields.forEach(field => {
                        let code = field.dataset.code;
