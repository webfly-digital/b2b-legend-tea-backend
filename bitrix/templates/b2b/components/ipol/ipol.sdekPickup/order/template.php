...
if (!$linkNamePVZ) $linkNamePVZ = 'Выбрать ПВЗ';
...
<script>
    var IPOLSDEK_pvz = {
        ...
        buttonPVZ: '<a href="javascript:void(0);" class="SDEK_selectPVZ button-full" onclick="IPOLSDEK_pvz.selectPVZ(\'#id#\',\'PVZ\'); return false;"><?=$linkNamePVZ?></a>',

        buttonPOSTAMAT: '<a href="javascript:void(0);" class="SDEK_selectPVZ button-full" onclick="IPOLSDEK_pvz.selectPVZ(\'#id#\',\'POSTAMAT\'); return false;"><?=$linkNamePOSTAMAT?></a>',
                ...
                for (var j in IPOLSDEK_pvz.deliveries[i]) {
                    ...
                    if (IPOLSDEK_pvz.deliveries[i][j].self) {
                       ...
                       if (
                           ...
                           let btnId = '#IPOLSDEK_injectHere-'+IPOLSDEK_pvz.deliveries[i][j].link;

                            if (!$(btnId).length)
                                $('#bx-soa-delivery').find('.bx-soa-pp-company-desc').after('<div id="'+btnId+'"></div>');
                            if ($(btnId).length == 0) {
        ...
        labelPzv: function (i, mode) {
            if (typeof(IPOLSDEK_pvz.deliveries[mode][i]) === 'undefined')
                return false;
            //custom start
            var tmpHTML = "<div class='sdek_pvzLair'>" + IPOLSDEK_pvz['button' + mode].replace('#id#', i);
        ...
        choozePVZ: function (pvzId, isAjax) {
            ...
            if (typeof isAjax === 'undefined') {
                ...
                if (typeof IPOLSDEK_DeliveryChangeEvent === 'function')
                    ...
                    //custom start
                    if (window.location.href.indexOf('test.php') == -1) {
                        if (typeof(BX.Sale) !== 'undefined' && BX.WebflyCheckoutComponent !== 'undefined') {
                            BX.WebflyCheckoutComponent.sendRequest();
                        }
                    } else {
        ...
        markUnable: function () {
            for (var i in IPOLSDEK_pvz.pvzInputs) {
                ...
                if (chznPnkt.length > 0) {
                    ...
                    //chznPnkt.attr('readonly', 'readonly');
                    break;
                }
            }
        },
