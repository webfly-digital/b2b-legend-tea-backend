...
foreach ($xml as $data) {
    ...
    foreach($data[0]->КонтрагентыПартнера->Контрагент as $k => $val) { //
        if (intval(trim($val->ID))>0) {
            $arContrIds[trim(str_replace(" ", "", $val->ID))] = trim(str_replace(" ", "", $val->ID));
...
foreach ($arContrIds as $id) {
    $db_sales = CSaleOrderUserProps::GetList(array("ID" => "ASC"), array("ID" => $id));
    while ($ar_sales = $db_sales->Fetch()) {
        ...
        if ($arGUIDSCA[$ar_sales["ID"]]) {
            $arFieldsProps = array(
                "USER_PROPS_ID" => $ar_sales["ID"],
                "NAME" => "1C GUID",
                "VALUE" => $arGUIDSCA[$ar_sales["ID"]]
            );
            if ($ar_sales["PERSON_TYPE_ID"]==5) {
                $arFieldsProps["ORDER_PROPS_ID"] = 110;
            }
            if ($ar_sales["PERSON_TYPE_ID"]==6) {
                $arFieldsProps["ORDER_PROPS_ID"] = 111;
            }

            echo "<br> ssfd ".CSaleOrderUserPropsValue::Add($arFieldsProps);
        }

        echo $arGUIDSCA[$ar_sales["ID"]]."<pre>";
        print_r($ar_sales);
        echo "</pre>";
...
while ($arUser = $rsUsers->GetNext()) {
        ...
        $contact_fio = $contact_email = $contact_phone = $contact_id = $contr_id = '';
        ...
        while ($ar_sales = $db_sales->Fetch()) {
            ...
            if ($ar_sales["PERSON_TYPE_ID"] == 1 or $ar_sales["PERSON_TYPE_ID"] == 6) {
                $xml .= '<Контрагент>
                    <GUID_Контрагент>'.$arGUIDSCA[$ar_sales["ID"]].'</GUID_Контрагент>
                    <ID>' . $ar_sales["ID"] . '</ID>
